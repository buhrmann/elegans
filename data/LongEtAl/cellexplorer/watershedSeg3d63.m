function [outsegImg, threImg] = WatershedSeg3d63(segImg, inimg, idx, rgnMinSize, rgnConvexityLow, threImg, imgproptag, method,rgnMaxiSize)
% function [outsegImg, threImg] = WatershedSeg3d63(segImg, inimg, idx, rgnMinSize, rgnConvexityLow, threImg, imgproptag, method,rgnMaxiSize)

% segment nuclei using geometry-based watershed segmentation and
% region grouping
% 
% Copyright F. Long
% Dec.5, 2006
...
for i=1:length(idx)
    ...
    holeimgrgn = uint16(inimg(yrange, xrange, zrange));  
    ...
    if (rgnnum>1) %otherwise, there is no need to merge
          f =  rgnMerge64(f, inimgrgn, holeimgrgn, rgnMaxiSize);%20061205          
    end; %if (rgnnum>1) end
